...
class CameraInfo(NamedTuple):
    ...
    edit_image: Optional[np.array] = None
...
def readNerfiesCameras(path):
    ...
    # first for editing image, then all_img will be new path
    if os.path.isdir(f'{path}/edit'):
        all_edit_img = [f'{path}/edit/{int(1 / ratio)}x/{i}.png' for i in all_img]
    else:
        all_edit_img = None
    ...
    for idx in range(len(all_img)):
        ...
        if all_edit_img is not None:
            edit_image_path = all_edit_img[idx]
            edit_image = np.array(Image.open(edit_image_path))
            edit_image = Image.fromarray((edit_image).astype(np.uint8))
        else:
            edit_image = None
        ...
        cam_info = CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image, edit_image=edit_image,
                              image_path=image_path, image_name=image_name, width=image.size[
                                  0], height=image.size[1],
                              fid=fid)
        cam_infos.append(cam_info)
