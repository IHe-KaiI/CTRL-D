...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask, image_name, uid,
                 trans=np.array([0.0, 0.0, 0.0]), scale=1.0, data_device="cuda", fid=None, depth=None, edit_image=None):
        ...
        self.edit_image = edit_image.clamp(0.0, 1.0).to(self.data_device) if edit_image is not None else None
        ...
        if gt_alpha_mask is not None:
            ...
            if self.edit_image is not None: self.edit_image *= gt_alpha_mask.to(self.data_device)
        else:
            self.original_image *= torch.ones((1, self.image_height, self.image_width), device=self.data_device)
            if self.edit_image is not None: self.edit_image *= torch.ones((1, self.image_height, self.image_width), device=self.data_device)
    ...
    def load2device(self, data_device='cuda'):
        ...
        self.edit_image = self.edit_image.to(data_device) if self.edit_image is not None else None
